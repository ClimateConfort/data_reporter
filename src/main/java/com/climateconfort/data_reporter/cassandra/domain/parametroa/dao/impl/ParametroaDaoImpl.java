...
public class ParametroaDaoImpl implements ParametroaDao
{
    ...
    @Override
    public List<Parametroa> findByGelaId(int gela_id) {
        String query = "SELECT * FROM Parametroa WHERE gela_id = ? ALLOW FILTERING";
        ResultSet result = session.execute(query, gela_id);
        ...
        for (Row row : result.all())
        {
            ...
            parametroa.setBalio_max(row.getFloat("balio_max"));
            parametroa.setBalio_min(row.getFloat("balio_min"));
